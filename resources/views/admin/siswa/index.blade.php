...
@section('content')
    ...
    <div class="w-full mx-auto px-4 lg:px-8 py-5 flex-grow space-y-5">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="space-y-1.5">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-0.5">Kelola data seluruh siswa terdaftar, NISN, dan kelas.</p>
                </div>
            </div>
            <div>
                <a href="{{ route('admin.siswa.create') }}" class="inline-flex items-center space-x-2 bg-orange-500 hover:bg-orange-600 text-white text-xs font-semibold px-4 py-2.5 rounded-xl shadow-md shadow-orange-500/20 transition-all">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-slate-800 mt-1">{{ $totalSiswa }}</h3>
            ...
            <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
                <div>
                    <p class="text-xs font-medium text-slate-400">Siswa Aktif</p>
                    <h3 class="text-2xl font-bold text-slate-800 mt-1">{{ $totalSiswaAktif }}</h3>
                    <p class="text-[11px] text-slate-400 mt-0.5">dari {{ $totalSiswa }} Terdaftar</p>
                </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <form method="GET" action="{{ route('admin.siswa.index') }}" class="lg:col-span-3 bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                <div class="flex items-center justify-between pb-3 border-b border-slate-100">
                    ...
                    <a href="{{ route('admin.siswa.index') }}" class="text-xs text-orange-600 hover:underline font-semibold">Reset</a>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        <input type="text" name="search" value="{{ $search }}" placeholder="Nama atau NISN..." class="w-full text-xs pl-8 pr-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-1 focus:ring-orange-500">
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="status" class="w-full text-xs px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-1 focus:ring-orange-500 text-slate-600">
                        ...
                        <option value="lulus" {{ $statusTerpilih == 'lulus' ? 'selected' : '' }}>Lulus / Alumni</option>
                    </select>
                </div>
                                ...
                                <tr class="hover:bg-slate-50/50">
                                    ...
                                    <td class="py-3.5 px-4">
                                        <div class="flex items-center space-x-3">
                                            <div class="w-8 h-8 rounded-full bg-emerald-100 text-emerald-700 overflow-hidden shrink-0 flex items-center justify-center font-bold text-xs">
                                                {{ $inisial }}
                                            </div>
                                            <p class="font-bold text-slate-800">{{ $s->name }}</p>
                                        </div>
                                    </td>
                                    <td class="py-3.5 px-4 text-slate-600 font-medium">{{ $s->nis }}</td>
                                    <td class="py-3.5 px-4 font-semibold text-slate-700">{{ $s->kelas }}</td>
                                    <td class="py-3.5 px-4">
                                        @if($s->status === 'aktif')
                                        ...
                                        @else
                                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium bg-slate-100 text-slate-500">
                                                <span class="w-1.5 h-1.5 rounded-full bg-slate-400 mr-1.5"></span> Lulus
                                            </span>
                                        @endif
                                    </td>
                                    <td class="py-3.5 px-4 text-center">
                                        <div class="flex items-center justify-center space-x-2">
                                            <a href="#" class="w-7 h-7 rounded-lg bg-orange-50 text-orange-600 hover:bg-orange-100 flex items-center justify-center border border-orange-200 transition-colors"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg></a>
                                            <a href="{{ route('admin.siswa.edit', $s->id) }}" class="w-7 h-7 rounded-lg bg-amber-50 text-amber-600 hover:bg-amber-100 flex items-center justify-center border border-amber-200 transition-colors"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></a>
                                            <form action="{{ route('admin.siswa.destroy', $s->id) }}" method="POST" onsubmit="return confirm('Hapus data siswa {{ $s->name }}?');">
                                                @csrf
                                                @method('DELETE')
                    ...
                    <div class="flex items-center space-x-3">
                        <div class="flex items-center space-x-1">
                            <a href="{{ $siswa->previousPageUrl() ?? '#' }}"
                               class="w-7 h-7 rounded-lg border border-slate-200 flex items-center justify-center hover:bg-slate-50 {{ $siswa->onFirstPage() ? 'opacity-40 pointer-events-none' : '' }}">&lt;</a>

                            @foreach(range(1, $siswa->lastPage()) as $page)
                                <a href="{{ $siswa->url($page) }}"
                                   class="w-7 h-7 rounded-lg flex items-center justify-center font-semibold {{ $page == $siswa->currentPage() ? 'bg-orange-500 text-white shadow-sm' : 'border border-slate-200 hover:bg-slate-50 text-slate-600' }}">
                                    {{ $page }}
                                </a>
                            @endforeach

                            <a href="{{ $siswa->nextPageUrl() ?? '#' }}"
                               class="w-7 h-7 rounded-lg border border-slate-200 flex items-center justify-center hover:bg-slate-50 {{ $siswa->hasMorePages() ? '' : 'opacity-40 pointer-events-none' }}">&gt;</a>
                        </div>
                        <div>
