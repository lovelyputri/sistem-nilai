...
<main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <nav class="flex items-center gap-2 text-xs font-medium text-slate-400 mb-4">
        ...
        <i data-lucide="chevron-right" class="w-3.5 h-3.5"></i>
        <span class="text-orange-600 font-semibold">Tambah Guru</span>
    </nav>

    <!-- Page Title Header -->
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Tambah Data Guru</h1>
            <p class="text-sm text-slate-500 mt-1">Daftarkan akun pendidik baru ke dalam portal sistem nilai sekolah.</p>
        </div>
        <div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="px-6 py-4 bg-slate-50/80 border-b border-slate-100 flex items-center gap-3">
            ...
            <div>
                <h2 class="text-base font-bold text-slate-800">Formulir Pendaftaran Guru Baru</h2>
                <p class="text-xs text-slate-400">Isi data lengkap guru di bawah ini.</p>
            </div>
        </div>

        <!-- Card Body Form -->
        <form method="POST" action="{{ route('admin.guru.store') }}" class="p-6 sm:p-8 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" placeholder="Contoh: Budi Santoso, S.Pd." required
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        ...
                        <input type="email" name="email" value="{{ old('email') }}" placeholder="guru@sekolah.sch.id" required
                            class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 focus:bg-white focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 outline-none transition-all">
                    </div>
                    @error('email')
                        <p class="text-xs text-red-500 flex items-center gap-1 mt-1">
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="nip" value="{{ old('nip') }}" placeholder="198203152008011002" required
                            class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 focus:bg-white focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 outline-none transition-all">
                    </div>
                    @error('nip')
                        <p class="text-xs text-red-500 flex items-center gap-1 mt-1">
            ...
            <div class="p-4 rounded-xl bg-amber-50 border border-amber-200 flex items-start gap-3 text-amber-900">
                ...
                <p class="text-xs leading-relaxed">
                    Secara bawaan, akun baru akan langsung disinkronkan dengan portal guru. Anda dapat mengaktifkan atau meninjau status verval akun pada menu <strong>Kelola Guru</strong> kapan saja.
                </p>
            </div>
