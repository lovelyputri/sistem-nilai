...
<main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <nav class="flex items-center gap-2 text-xs font-medium text-slate-400 mb-4">
        ...
        <a href="{{ route('admin.guru.index') }}" class="hover:text-slate-600 transition-colors">Guru</a>
        <i data-lucide="chevron-right" class="w-3.5 h-3.5"></i>
        <span class="text-orange-600 font-semibold">Edit Guru</span>
    </nav>

    <!-- Page Title Header -->
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Edit Data Guru</h1>
            <p class="text-sm text-slate-500 mt-1">Perbarui informasi akun pendidik dalam portal sistem nilai sekolah.</p>
        </div>
        <div>
            <a href="{{ route('admin.guru.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 font-semibold text-sm shadow-sm transition-all">
    ...
    <!-- Global Error Banner (Laravel Validation) -->
    @if($errors->any())
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="px-6 py-4 bg-slate-50/80 border-b border-slate-100 flex items-center gap-3">
            ...
            <div>
                <h2 class="text-base font-bold text-slate-800">Formulir Edit Data Guru</h2>
                <p class="text-xs text-slate-400">Perbarui data guru di bawah ini lalu simpan perubahan.</p>
            </div>
        </div>

        <!-- Card Body Form -->
        <form method="POST" action="{{ route('admin.guru.update', $guru->id) }}" class="p-6 sm:p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="name" value="{{ old('name', $guru->name) }}" placeholder="Contoh: Budi Santoso, S.Pd." required
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        ...
                        <input type="email" name="email" value="{{ old('email', $guru->email) }}" placeholder="guru@sekolah.sch.id" required
                            class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 focus:bg-white focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 outline-none transition-all">
                    </div>
                    @error('email')
                        <p class="text-xs text-red-500 flex items-center gap-1 mt-1">
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="nip" value="{{ old('nip', $guru->nip) }}" placeholder="198203152008011002" required
                            class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 focus:bg-white focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 outline-none transition-all">
                    </div>
                    @error('nip')
                        <p class="text-xs text-red-500 flex items-center gap-1 mt-1">
                ...
                <div class="space-y-1.5">
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider">
                        Mata Pelajaran Utama <span class="text-red-500">*</span>
                    </label>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                        ...
                        <select name="id_mata_pelajaran" required
                            ...
                            @if(isset($mataPelajaran))
                                ...
                                @endforeach
                            @else
                                <option value="1">Produk Kreatif dan Kewirausahaan</option>
                                <option value="2">Pendidikan Agama Islam</option>
                                <option value="3">Pengembangan Perangkat Bergerak</option>
                ...
                <div class="space-y-1.5">
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider">
                        Kata Sandi <span class="text-slate-400 font-normal lowercase">(opsional)</span>
                    </label>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                        ...
                        <button type="button" onclick="togglePassword('password')" class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-slate-600">
                            <i data-lucide="eye" id="password-icon" class="w-4 h-4"></i>
                        </button>
                    </div>
                    @error('password')
                        <p class="text-xs text-red-500 flex items-center gap-1 mt-1">
                ...
                <div class="space-y-1.5">
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider">
                        Konfirmasi Kata Sandi <span class="text-slate-400 font-normal lowercase">(opsional)</span>
                    </label>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                        ...
                        <button type="button" onclick="togglePassword('password_confirmation')" class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-slate-600">
                            <i data-lucide="eye" id="password_confirmation-icon" class="w-4 h-4"></i>
                        </button>
                    </div>
                </div>
            ...
            <div class="p-4 rounded-xl bg-amber-50 border border-amber-200 flex items-start gap-3 text-amber-900">
                ...
                <p class="text-xs leading-relaxed">
                    Biarkan bidang <strong>Kata Sandi</strong> dan <strong>Konfirmasi Kata Sandi</strong> kosong jika Anda tidak ingin mengubah kata sandi guru.
                </p>
            </div>
            ...
            <div class="pt-4 border-t border-slate-100 flex flex-col-reverse sm:flex-row items-center justify-end gap-3">
                <button type="reset" class="w-full sm:w-auto px-5 py-2.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 font-semibold text-sm transition-all flex items-center justify-center gap-2">
                    <i data-lucide="rotate-ccw" class="w-4 h-4"></i>
                    Reset Form
                </button>
                <button type="submit" class="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-orange-500 hover:bg-orange-600 active:bg-orange-700 text-white font-semibold text-sm shadow-md shadow-orange-500/25 transition-all flex items-center justify-center gap-2">
